...
@section('main-content')
    <section class="content section-form">
            <form class="form-horizontal" method="post" action="{{ route('utilisateur.update', [$modifs->id]) }}" enctype="multipart/form-data" onsubmit =" return confirmAdd();">
                 {{csrf_field()}}
                 {{method_field('POST')}}
             ...
             @endif
              @if ($succesAdd==true)
               <div class="alert alert-success" role="alert">
                 <button type="button" class="close" data-dismiss="alert">&times;</button>
                  <span style="text-align: center; margin-left: 30% ">L'utilisateur à été bien ajouté dans la base</span>
               </div>
           @endif
           ...
           @if ($succesEdit==true)
               <div class="alert alert-success" role="alert">
                 <button type="button" class="close" data-dismiss="alert">&times;</button>
                  <span style="text-align: center; margin-left: 30% ">L'utilisateur à été bien modifié dans la base</span>
               </div>
           @endif
							<div class="active tab-pane" >
								...
								<div class="form-group">
									...
									<div class="col-md-4" >
									  <input type="text" class="form-control " name="nom" id="nom" placeholder="Nom" autocomplete="off" value="{{$modifs->nom}}" required>
									</div>
								</div>
								<div class="form-group">
									<label for="prenom" class="col-md-4 control-label">Prénom : </label>
									<div class="col-md-4">
									  <input type="text" class="form-control" name="prenom" id="prenom" placeholder="Prénom" autocomplete="off" value="{{$modifs->prenom}}" required>
									</div>
								</div>
								<div class="form-group">
									<label for="email" class="col-md-4 control-label">Email : </label>
									<div class="col-md-4">
									  <input type="email" class="form-control" name="email" id="email" placeholder="ex: x@y.z" autocomplete="off" value="{{$modifs->email}}" required>
									</div>
							</div>
						</div>
...
<div class="col-md-12">
	<div class="container">
	 ...
	 <div style="margin-left: 0%"><h3>Liste des utilisateurs</h3>
	<br/></div>
      ...
      <div  class="table-responsive box-body no-padding">
      	 <table class="table table-hover table-striped" id="tab-user">
      	 	  ...
      	 	  @foreach($utilisateurs as $utilisateur)
                  ...
                  <td> 
                  <form action = "{{route('utilisateur.destroy', [$utilisateur->id])}}" method="post" class="">
                            @csrf
                            @method('DELETE')
